    ...
    public class ManageMod
    {
        ...
        private Point GetFaninSPoint(string name)
        {
            ...
            if (ModDict.ContainsKey(modid))
            {
                ...
                foreach(FaninFanout ff in mod.faninfanouts)
                {
                    if(ff.fanin.name == c[1])
                    {
                        return ff.fanin;
                    }
                }
        ...
        private int InitFaninFanoutService(ModInfo mod)
        {
            return RunInstanceMethod(mod, "FaninFanoutService");
        }
            ...
            for (int i = 0; i < ModConst.MaxLevel; i++)
            {
                if (Initlevel.ContainsKey(i))
                {
                    ...
                    foreach (int j in idl)
                    {
                        ...
                        rlt = InitFaninFanoutService(mod);
                        if (rlt != 0)
                        {
                            Logging.logger.Error("Init Fanin Fanout failed " + mod.modname + " " + mod.cls);
                            continue;
                        }

                        rlt = InitFanWorksService(mod);
                        if (rlt != 0)
                        {
                            Logging.logger.Error("Init Fan work failed " + mod.modname + " " + mod.cls);
                            continue;
                        }

                        rlt = InitResource(mod);
                        if (rlt != 0)
                        {
                            Logging.logger.Error("Init Resource failed " + mod.modname + " " + mod.cls);
                            continue;
                        }
                    }
                }
