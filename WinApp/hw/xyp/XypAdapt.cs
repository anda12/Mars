...
namespace WinApp.hw.xyp
{
    ...
    {
        private bool NeedInitGlobal = true;
        public XypAdapt()
        {
            recvList = new List<string>();
            baselock = new object();
            recvlock = new object();
            baseRecvLock = new object();
            //InitRecviverAll();
            //InitBaseAll();
            //InitBaseRecvAll();
            //initBaseList();
        ...
        protected override int ModInifResourceInfo()
        {
            InitGlobal();
            return base.ModInifResourceInfo();
        }
        public int InitGlobal()
        {
            if (NeedInitGlobal)
            {
                InitBaseAll();
